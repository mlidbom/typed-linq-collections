...
class TestTakeWhileOperations:
    def test_take_while_returns_elements_until_predicate_becomes_false(self) -> None:
        assert query([1, 2, 3, 4, 5]).take_while(lambda x: x < 4).to_list() == [1, 2, 3]

    def test_take_while_with_always_true_predicate_returns_all_elements(self) -> None:
        assert query([1, 2, 3, 4, 5]).take_while(lambda x: x > 0).to_list() == [1, 2, 3, 4, 5]

    def test_take_while_with_always_false_predicate_returns_no_elements(self) -> None:
        assert query([1, 2, 3, 4, 5]).take_while(lambda x: x < 0).to_list() == []

    def test_take_while_with_first_element_failing_predicate_returns_empty(self) -> None:
        assert query([5, 1, 2, 3]).take_while(lambda x: x < 3).to_list() == []

    def test_take_while_from_empty_collection_returns_empty(self) -> None:
        assert query(list[int]([])).take_while(lambda x: True).to_list() == []

    def test_take_while_with_single_element_matching_predicate_returns_the_element(self) -> None:
        assert query([42]).take_while(lambda x: x > 0).to_list() == [42]

    def test_take_while_with_single_element_not_matching_predicate_returns_no_elements(self) -> None:
    ...
    def test_take_while_stops_processing_at_the_first_element_that_fails_the_predidate(self) -> None:
