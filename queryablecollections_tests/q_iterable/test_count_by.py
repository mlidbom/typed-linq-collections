...
def key_value_to_tuple[TKey, TValue](x: KeyValuePair[TKey, TValue]) -> tuple[TKey, TValue]:
    return (x.key, x.value)


def test_count_by_returns_count_of_each_key() -> None:
    lists_value_test([1, 2, 2, 3, 3, 3],
               lambda x: x.count_by(lambda y: y).select(key_value_to_tuple).to_list(),
               [(1, 1), (2, 2), (3, 3)])
...
def test_count_by_with_key_selector_function() -> None:
    lists_value_test(["apple", "apricot", "banana", "blueberry", "cherry"],
               lambda x: x.count_by(lambda word: word[0]).select(key_value_to_tuple).to_list(),
               [("a", 2), ("b", 2), ("c", 1)])

def test_count_by_with_length_selector() -> None:
    lists_value_test(["cat", "dog", "elephant", "ant", "bee"],
               lambda x: x.count_by(lambda word: len(word)).select(key_value_to_tuple).to_list(),
               [(3, 4), (8, 1)])
...
def test_count_by_with_boolean_selector() -> None:
    lists_value_test([1, 2, 3, 4, 5, 6],
               lambda x: x.count_by(lambda y: y % 2 == 0).select(key_value_to_tuple).to_list(),
               [(False, 3), (True, 3)])
...
def test_count_by_with_mixed_types() -> None:
    lists_value_test([1, "hello", 2.5, "hello", 1, True],
               lambda x: x.count_by(lambda y: type(y).__name__).select(key_value_to_tuple).to_list(),
               [("int", 2), ("str", 2), ("float", 1), ("bool", 1)])

def test_count_by_maintains_insertion_order() -> None:
    lists_value_test(["z", "a", "z", "b", "a", "z"],
               lambda x: x.count_by(lambda x: x).select(key_value_to_tuple).to_list(),
               [("z", 3), ("a", 2), ("b", 1)])

def test_count_by_can_be_chained_with_other_operations() -> None:
    lists_value_test([1, 2, 2, 3, 3, 3],
               lambda x: x.count_by(lambda x: x).where(lambda kv: kv.value > 1).select(key_value_to_tuple).to_list(),
               [(2, 2), (3, 3)])
...
def test_count_by_with_complex_objects() -> None:
    ...
    lists_value_test(people,
               lambda x: x.count_by(lambda p: p.age).select(key_value_to_tuple).to_list(),
               [(25, 2), (30, 2)])

def test_count_by_returns_proper_key_value_pairs() -> None:
    result = query([1, 2, 2, 3]).count_by(lambda x: x).first()
