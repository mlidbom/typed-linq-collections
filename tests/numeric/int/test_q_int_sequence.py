...
from typed_linq_collections.collections.numeric.q_int_types import QIntIterable, QIntSequence
...
def test_min_or_default_returns_min_of_the_values() -> None:
    assert QIntSequence([6, 2, 5, 3]).min_or_default() == 2

def test_min_or_default_returns_0_on_on_empty_collection() -> None:
    assert QIntSequence().min_or_default() == 0

def test_max_returns_max_of_the_values() -> None:
    assert QIntSequence([1, 5, 3]).max() == 5
...
def test_max_or_default_returns_max_of_the_values() -> None:
    assert QIntSequence([1, 5, 3]).max_or_default() == 5

def test_max_or_default_returns_0_on_on_empty_collection() -> None:
    assert QIntSequence().max_or_default() == 0

def test_average_returns_average_of_the_values() -> None:
    assert QIntSequence([1, 2, 3]).average() == 2
...
def test_average_or_default_returns_average_of_the_values() -> None:
    assert QIntSequence([1, 2, 3]).average_or_default() == 2

def test_average_or_default_returns_0_on_on_empty_collection() -> None:
    assert QIntSequence().average_or_default() == 0
...
def test_reversed_returns_qint_iterable() -> None:
    result = QIntSequence([1, 2, 3]).reversed()
