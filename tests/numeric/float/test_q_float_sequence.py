...
def test_sum_returns_sum_of_the_values() -> None:
    assert QFloatSequence([1.1, 2.1, 3.1]).sum() == pytest.approx(6.3)  # pyright: ignore [reportUnknownMemberType]

def test_sum_returns_zero_on_on_empty_collection() -> None:
    assert QFloatSequence().sum() == 0

def test_min_returns_min_of_the_values() -> None:
    assert QFloatSequence([6.1, 2.1, 5.1, 3.1]).min() == 2.1
...
def test_min_or_default_returns_min_of_the_values() -> None:
    assert QFloatSequence([6.1, 2.1, 5.1, 3.1]).min_or_default() == 2.1

def test_min_or_default_returns_0_on_on_empty_collection() -> None:
    assert QFloatSequence().min_or_default() == 0

def test_max_returns_max_of_the_values() -> None:
    assert QFloatSequence([1.1, 5.1, 3.1]).max() == 5.1
...
def test_max_or_default_returns_max_of_the_values() -> None:
    assert QFloatSequence([1.1, 5.1, 3.1]).max_or_default() == 5.1

def test_max_or_default_returns_0_on_on_empty_collection() -> None:
    assert QFloatSequence().max_or_default() == 0

def test_average_returns_average_of_the_values() -> None:
    assert QFloatSequence([1.1, 2.1, 3.1]).average() == 2.1
...
def test_average_or_default_returns_average_of_the_values() -> None:
    assert QFloatSequence([1.1, 2.1, 3.1]).average_or_default() == 2.1

def test_average_or_default_returns_0_on_on_empty_collection() -> None:
    assert QFloatSequence().average_or_default() == 0

def test_to_sequence_returns_a_sequence_with_the_same_elements() -> None:
    assert QFloatSequence([1.1, 2.1, 3.1]).to_sequence().to_list() == [1.1, 2.1, 3.1]

def test_constructor_with_no_arguments_creates_empty_collection() -> None:
    empty_sequence = QFloatSequence()
    assert len(empty_sequence) == 0
    assert list(empty_sequence) == []

def test_reversed_returns_qfloat_iterable() -> None:
    result = QFloatSequence([1.1, 2.1, 3.1]).reversed()
    # The reversed method returns QFloatIterable (not QFloatSequence specifically)
